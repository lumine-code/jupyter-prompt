...
class PromptPanel {
  ...
  constructor(getKernel) {
    ...
    this.selectList = lumine.workspace.buildSelectList({
      ...
      confirmAction: (entry) => {
        if (entry) return "jupyter-prompt:run-history-entry";
        return this.selectList.getQuery().trim() ? "jupyter-prompt:run-prompt" : null;
      },
    ...
    // registrations and the keymap, so nothing is documented twice. Enter
    // remains core:confirm; confirmAction only displays it beside the semantic
    // run command for the current prompt state.
  ...
  actionAppliesToSelection(command) {
    const entry = this.selectList.getSelectedItem();
    if (command === "jupyter-prompt:run-prompt") {
      return !entry && Boolean(this.selectList.getQuery().trim());
    }
  ...
  recallSelection() {
    ...
    this.selectList.getQueryEditor().setText(entry.code);
